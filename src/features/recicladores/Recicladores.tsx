...
import RecicladoresTable, { type Reciclador } from './RecicladoresTable'
...
const fetchRecicladores = async (): Promise<Reciclador[]> => {
  // Mock — reemplazar con: apiClient.get('/api/v1/users?user_type=recycler')
  await new Promise((r) => setTimeout(r, 800))
  return MOCK_RECICLADORES
}

export default function Recicladores() {
  const { data = [], isLoading } = useQuery({ queryKey: ['recicladores'], queryFn: fetchRecicladores })

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <Box>
        <Typography variant="h5" fontWeight={600}>Recicladores</Typography>
        <Typography variant="body2" color="text.secondary" mt={0.5}>Recicladores asociados a esta ECA</Typography>
      </Box>
